...
8. [テクニカルバックグラウンド](#テクニカルバックグラウンド)
   ...
   - [11. ソフト決定（消去）復号と構造化インタリーブ（v3.0.0+）](#11-ソフト決定消去復号と構造化インタリーブv300)
...
| **[共通]** 誤り訂正 | リード・ソロモン符号 (ECC) + **消去復号 (v3.0.0+)** | 自己修復、訂正能力最大 2 倍 |
| **[共通]** 空間スクランブル | Arnold変換 + **構造化インタリーブ (v3.0.0+)** | 解析耐性 + バースト誤り耐性 |

### v3.0.0 — 堅牢性アップデート（破壊的変更）

- **ソフト決定（消去）復号**: 抽出時の `bitSums` 信頼度から低信頼度シンボルを「消去」と宣言し、`2·errors + erasures ≤ eccLen` の境界で復号。同じ ECC シンボル数で**訂正能力が最大 2 倍**になり、JPEG 重圧縮や Q≈70 程度でも長めペイロード（30〜32 chars）が実用域に入ります
- **構造化ビットインタリーブ**: シンボル内の 6 ビットを `b * 63 + s` のビットプレーン優先で配置。同一シンボルの 2 ビットが bitMatrix 上で常に 63 位置以上離れるため、Arnold スクランブルと相乗してバースト誤り耐性が向上
- **互換性**: v2.x で生成したウォーターマーク画像は v3.x では復号できません（ビット配置が変更されたため）。既存ウォーターマークを読みたい場合は v2.x を pin してください
- **画像の知覚的劣化はゼロ**: 上記改善は符号化レイヤとデコーダ側のみの変更で、埋め込みエネルギー (delta) や対象ブロックは v2.x と同一
...
| `regions` | number | `1` | **空間ダイバーシティ（複数領域埋め込み）**。画像を N 個の領域に分割し、同じウォーターマークを各領域に埋め込みます。抽出時は領域ごとのソフト値を合算（Maximal-Ratio Combining）します。局所的な画像劣化や部分的なクロップへの耐性が向上します。各領域に最低 **160×160 px** 必要です。画像サイズ不足時は自動的に入る最大値に縮退します（`1` まで）。埋め込みと抽出で**必ず同じ値**にする必要があります。詳細は [空間ダイバーシティ](#空間ダイバーシティregions) 参照。 |
| `softDecoding` | boolean | `true` | **ソフト決定（消去）復号** (v3.0.0+)。低信頼度のシンボル位置を「消去」として扱い、Reed-Solomon の訂正能力を最大 **2 倍**に拡張します。`2 × errors + erasures ≤ eccLen` の境界で動作するため、JPEG 圧縮など多数のビット誤りが発生する状況で堅牢性が大幅に向上します。`false` でハード決定復号にフォールバック。 |
| `erasureThreshold` | number | `0.35` | **消去判定の相対閾値** (`softDecoding: true` のときのみ有効)。各シンボルの最小ビット信頼度が「全シンボル信頼度の中央値 × この値」を下回ると消去とみなします。大きくすると消去が増えて弱いビットを救いやすくなりますが、ECC 容量を多く消費します。 |
...
### 2. Reed-Solomon 誤り訂正符号（ECC）

#### 概要

バーコード・QRコード・CD の誤り訂正にも使われる代数的符号理論。本ライブラリでは **GF(2^6) = GF(64) 上の RS 符号** (`ReedSolomonGF64`) を採用しています。

**なぜ GF(64) か:**
透かしのペイロードは Base64url 文字列（`A-Z, a-z, 0-9, -, _` の 64 文字）で表現されます。GF(64) は 1 シンボル = 6 bit = Base64url 1 文字に正確に対応するため、シンボル境界とペイロード境界が一致し、変換時のロスがありません（GF(2^8) = 8 bit シンボルを使うと境界が合わずパディング浪費が発生します）。

**フォレンジック透かしでの設定（デフォルト）:**
- データシンボル: 22 (= 22 chars Base64url)
- ECC シンボル:    41
- コードワード長: 63 シンボル (GF(64) の最大値、= 378 bit)
- ハード訂正能力: 最大 20 シンボル誤り
- ソフト訂正能力 (v3.0.0+): 最大 41 シンボル消去 (`2·errors + erasures ≤ 41`)

**FSK 透かしでの設定（デフォルト）:**
- データシンボル: 22
- ECC シンボル:    18
- コードワード長: 40 シンボル (FSK は固定 40 シンボル)
- ハード訂正能力: 最大 9 シンボル誤り

#### 誤り訂正のメカニズム

シンドローム多項式から誤り位置多項式を Berlekamp-Massey 法で計算し、Chien サーチで誤り位置を、Forney 公式で誤りの値を導出して修復します。実装は `watermark-lib/src/rs-gf64.ts` にゼロ依存で記述されており、ブラウザ・Node.js のどちらでも動作します。

**v3.0.0+ の errors-and-erasures 復号:**
抽出時のソフト値 (`bitSums`) から信頼度の低いシンボル位置を特定し、消去 (erasure) として宣言してから復号します。RS の理論的限界 `2 × errors + erasures ≤ eccLen` により、純粋な消去のみであれば**ハード復号の 2 倍**のシンボルを訂正できます。

**得意・不得意:**

**得意:**
- バースト誤り（連続した破損）への対応 — 構造化インタリーブ (v3.0.0+) でさらに耐性向上
- ランダム誤り（点在する破損）への対応
- ソフト復号（消去訂正）対応 (v3.0.0+) — JPEG 圧縮等で発生するビット信頼度の低下を直接訂正

**不得意:**
- 誤りバイト数が訂正能力を超えた場合は `null` を返す（訂正不可）
- マーカー (16 bit) の検出が完全に失敗するレベルの劣化（同期そのものが取れない）
...
### 11. ソフト決定（消去）復号と構造化インタリーブ（v3.0.0+）

v3.0.0 で導入された **2 つの符号化レイヤ改善**。画像の埋め込みエネルギーや対象ブロックは v2.x と完全に同一（=知覚的劣化ゼロ）でありながら、JPEG 圧縮等への堅牢性を大幅に向上させます。

#### 11.1 ソフト決定（消去）復号

##### 概要

従来 (v2.x) の抽出は、各論理ビット位置での `bitSums[i]` を **ハード決定** (`> 0 ? 1 : 0`) で 1 / 0 に固定してから RS 復号に渡していました。これは情報の捨てすぎで、`|bitSums[i]|` が**確信度**を表す貴重な情報を破棄していました。

v3.0.0 ではこの確信度を活用し、低信頼度のシンボル位置を **消去 (erasure)** として宣言してから errors-and-erasures 復号を実行します。

##### RS の理論的境界

| モード | 訂正可能数の式 | ECC=41 の場合 |
| :--- | :--- | :--- |
| ハード復号 (v2.x) | `2 × errors ≤ eccLen` → `errors ≤ ⌊eccLen/2⌋` | **20 シンボル** |
| 消去のみ復号 | `erasures ≤ eccLen` | **41 シンボル** |
| 混合復号 (v3.0.0+) | `2 × errors + erasures ≤ eccLen` | 例: 5 errors + 31 erasures |

##### アルゴリズム概要 (`ReedSolomonGF64.decodeWithErasures`)

```
入力: 受信語 r, 消去位置リスト E = {e_0, ..., e_{ν-1}}
...
3. Forney シンドローム T(x) = σ_e(x) · S(x) mod x^d:
     T_j = Σ σ_e[i] · S_{j-i}

4. T[ν..d-1] に対して Berlekamp-Massey を実行
     → 誤り位置多項式 σ_E(x) を求める

5. 結合位置多項式 σ(x) = σ_e(x) · σ_E(x)

6. Chien サーチ: σ の根 → 全エラー/消去位置を確定

7. Forney 公式: 各位置の誤り値を計算し XOR で訂正
```

##### 消去判定の閾値設計

抽出時の `bitSums` 配列は Arnold スクランブルされた論理位置上にあります。これを `inverseArnoldFloat()` で原配置に戻し、各シンボル位置の最小ビット信頼度を計算します:

```
シンボル s の確信度 = min_{b ∈ [0, 5]} |unscrambledSoft[bitPosition(s, b)]|

中央値 m = median(全シンボル確信度)
消去判定 = 確信度 < m × erasureThreshold

容量超過時は確信度の低い順に eccLen 個まで採用
```

`erasureThreshold` のデフォルト 0.35 は典型的な JPEG 劣化（Q=50〜80）でハード復号失敗ケースを救済しつつ、過剰消去を避けるバランスポイント。中央値ベースなので画像のテクスチャ依存の信頼度スケール変動に頑健です。

##### 注意点

- 消去判定が完全に正確であれば理論上限の 2 倍訂正能力を発揮するが、現実には誤判定（正しいビットを消去と誤宣言）が発生するため実効値は理論値より低い（典型的に 1.3〜1.7 倍）
- 高 SNR 条件（軽い JPEG・PNG）では消去候補がほぼゼロのため自動的にハード復号と同等動作
- ハード復号で復号可能な場合、ソフト復号も同じ結果を返す（後方互換性 OK）

#### 11.2 構造化ビットインタリーブ

##### 概要

v2.x のビット配置は「シンボル s のビット b」を `MARKER.length + s × 6 + b` の位置に**逐次配置**していました。同一シンボルの 6 ビットが隣接位置に並ぶため、Arnold スクランブル後でもまれに空間的に近い 8×8 ブロックに集約される確率が残っていました。

v3.0.0 では「**ビットプレーン優先**」配置に変更:

```
v2.x: 位置 = MARKER + s × 6 + b   (シンボル順、6 ビットが隣接)
v3.0.0+: 位置 = MARKER + b × 63 + s   (ビットプレーン順、6 ビットが 63 位置以上分散)
```

##### 効果

- 同一シンボルの任意 2 ビット間距離が **常に 63 位置以上**（=Arnold 前で 75% 以上の最大可能距離）
- Arnold スクランブルと相乗して、JPEG の 8×8 ブロック単位の局所劣化が**1 シンボルの複数ビットを同時に破壊する確率を大幅低減**
- ハード復号でもソフト復号でも効果あり（特にバースト誤りに対して有効）

##### 実装

```typescript
// 単一の関数 bitPosition で双方向に変換
...
ループ順序の変更のみでメモリ・計算量に変化なし。Arnold/逆 Arnold 演算回数も同じ。

#### 11.3 v2.x との互換性

v3.0.0 の埋め込みウォーターマークは v2.x の抽出器では復号できません。逆も同様です:

| 埋め込みバージョン | 抽出バージョン | 結果 |
| :---: | :---: | :--- |
| v2.x | v2.x | 正常 |
| v2.x | v3.0.0 | **失敗** (ビット配置不一致) |
| v3.0.0 | v2.x | **失敗** (ビット配置不一致) |
| v3.0.0 | v3.0.0 | 正常 |

レガシー透かしを読みたい場合は v2.x を pin する運用を推奨。
...
HMACの強度式（GF64）：`HMAC強度 = (payloadSymbols - secureIdLength) × 6 ビット`

以下の表は **payloadSymbols=22（デフォルト）** 前提です。`payloadSymbols` を変更する場合は後述の [3パラメータ協調設計](#3パラメータ協調設計regions--payloadsymbols--secureidlength) も参照してください。

| secureIdLength | IDシンボル | HMACシンボル | HMAC強度 | 安全性評価 |
| :---: | :---: | :---: | :---: | :--- |
| 4 | 4 | 18 | **108ビット** | セキュリティ優先 |
| **6（デフォルト）** | **6** | **16** | **96ビット** | **推奨：実用バランス型** |
| 8 | 8 | 14 | 84ビット | 長いIDが必要な場合の上限（MAC基準充足） |
| 10 | 10 | 12 | 72ビット | MAC truncation 基準 (64bit) 超え・余裕縮小 |
| 12 | 12 | 10 | 60ビット | **非推奨**：MAC基準未満、10¹² 回/秒の攻撃で約36日で突破可能 |
| ≥14 | ≥14 | ≤8 | ≤48ビット | **危険**：GPU1台で数分〜数日で突破可能 |

**デフォルト値 secureIdLength=6 を変更すべきでない理由（暗号論的根拠）：**

- **96ビット強度** は NIST SP 800-107r1 が定める MAC トランケーション基準（64ビット以上）の1.5倍を確保
- ID空間は 64^6 ≈ 687億 通り。実用上のセッションID管理には十分
- 偽造コスト：2^96 ≈ 7.9 × 10²⁸ 回の試行が必要。10¹² 回/秒のGPUクラスタでも約 **25億年**（宇宙年齢の約1/5）

> **重要:** 埋め込み時と検証時で `secureIdLength` を必ず一致させてください。不一致では検証が必ず失敗します。

### 3パラメータ協調設計（regions × payloadSymbols × secureIdLength）

ID長を伸ばしつつ HMAC 強度を維持したい場合、次の3パラメータを協調して設定します:

```
HMAC強度       = (payloadSymbols - secureIdLength) × 6 ビット
ID文字数        = secureIdLength
ECCシンボル数   = (63 - payloadSymbols)

実効訂正能力 (v3.0.0+):
  ハード:  floor(ECCシンボル数 / 2) … `softDecoding: false` 時のみ
  ソフト:  最大 ECCシンボル数      … デフォルト (`softDecoding: true`)
                                   ※ `2·errors + erasures ≤ eccLen` 境界

実効堅牢性 = 実効訂正能力
             + regions による空間ダイバーシティ利得
             + 構造化インタリーブによるバースト誤り耐性 (v3.0.0+ 自動適用)
```

#### v3.0.0 で何が変わるか

v2.x までは ECC=41 シンボルで「最大 20 シンボル訂正」が上限でした。v3.0.0 のソフト復号により、同じ ECC=41 で**最大 41 シンボルの消去**を訂正できます（実用的にはビット信頼度の閾値判定精度に依存し、典型 25〜35 シンボル相当）。

これにより設計の自由度が拡大します:

- **同じ堅牢性で ID をより長く** — payloadSymbols を 22 → 32 まで上げても、v2.x の payloadSymbols=22 と同等以上の JPEG 耐性が見込める
- **同じ ID 長でより堅牢に** — ID 長を変えなければ、v2.x で復号失敗していた重圧縮ケースも復号成功する確率が上がる
- **regions を増やす前にまず payloadSymbols を増やす** — regions=1 (画像分割なし) のままで ID を伸ばせる選択肢が増える

#### 設計パターン例 (v3.0.0+)

| 用途 | regions | payloadSymbols | secureIdLength | HMAC強度 | ID長 | ECC | 訂正能力 (ハード / ソフト) |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **デフォルト** | 1 | 22 | 6 | 96bit | 6文字 | 41 | 20 / **41** |
| ID 32文字（v3 推奨） | 1 | **32** | 10 | **132bit** | **10文字** | 31 | 15 / **31** |
| ID 拡張＋強化 (v2 互換) | 3 | 30 | 10 | 120bit | 10文字 | 33 | 16 / **33** |
| ID 最大・HMAC維持 | 3 | 32 | 12 | **120bit** | **12文字** | 31 | 15 / **31** |
| 超高耐性型（短 ID） | 4 | 15 | 4 | 66bit | 4文字 | 48 | 24 / **48** |
| 情報量最大 | 4 | 40 | 14 | **156bit** | **14文字** | 23 | 11 / **23** |
| **ID 重視 (v3 新)** | 1 | **40** | **16** | **144bit** | **16文字** | 23 | 11 / **23** |

> **「ID 重視 (v3 新)」パターン:** v3.0.0 のソフト復号を前提に、regions=1 でも ID 16 文字 + 144 bit HMAC を実現。ECC は 23 シンボルだがソフト復号で 23 erasure まで救えるため、JPEG Q≈70 の劣化条件下でも実用域。テストで `payloadSymbols=32` での Q=70 復号成功を確認済み。

#### 使用例: ID 10文字・HMAC強度UP（v3.0.0 新パターン）

```typescript
const options = {
  payloadSymbols: 32,   // v2.x では非推奨だったが、v3.0.0 のソフト復号で実用域
  // softDecoding: true (デフォルト)
};
...
#### 使用例: regions と組み合わせて最大堅牢性

```typescript
// 空間ダイバーシティ × ソフト復号 × 構造化インタリーブの三段構え
const options = {
  regions: 3,           // 部分クロップ・不均一劣化に強い
  payloadSymbols: 32,   // 32文字 ID
  // softDecoding: true (デフォルト)
};
```
...
- `regions > 1` の実効 ECC 削減効果は**空間的に不均一な劣化**で顕著です。完全に均一なノイズでは既存の modulo 冗長化ですでにカバーされているため、実際に削減可能な ECC シンボル数は画像・劣化シナリオ依存です
- ソフト復号の実効訂正数は理想上限 (= ECC シンボル数) よりやや低い値に落ち着きます。これは `erasureThreshold` の判定で「実は正しいビット」を消去と誤宣言する分が減算されるため。デフォルトの `0.35` は典型的な JPEG 劣化で良好なバランスを取りますが、過酷な条件では `0.4〜0.5` に上げると救済率が増えることがあります
- `payloadSymbols ≤ 10` のような超高耐性ゾーンでは、ハード復号でも十分すぎる ECC があるためソフト復号の追加効果は限定的です。逆に `payloadSymbols ≥ 30` のゾーンでこそソフト復号の恩恵が大きい
- 3パラメータ（`regions` / `payloadSymbols` / `secureIdLength`）は**埋め込みと抽出で全て一致**させてください。`softDecoding` / `erasureThreshold` は抽出時のみ作用するため一致不要
...
「最大誤り訂正」列はハード決定復号（`softDecoding: false`）の場合の値です。**v3.0.0 以降のデフォルト** (`softDecoding: true`) では、消去復号により `2 × errors + erasures ≤ eccLen` の境界まで訂正可能になります。理想条件下では訂正能力が最大 2 倍になり、JPEG 重圧縮・スクリーンショット等のビット誤りが多い経路でも復号成功率が大幅に向上します。

| payloadSymbols | ECCシンボル | ハード訂正 | 消去訂正上限 | ペイロード長 | 用途 |
| :---: | :---: | :---: | :---: | :---: | :--- |
| 10 | 53 | **26シンボル** | **53シンボル** | 10文字 | 超高耐性（圧縮・スクランブル攻撃対策） |
| 15 | 48 | **24シンボル** | **48シンボル** | 15文字 | 高耐性 |
| **22（デフォルト）** | **41** | **20シンボル** | **41シンボル** | **22文字** | **推奨：耐性とデータ量のバランス** |
| 30 | 33 | 16シンボル | **33シンボル** | 30文字 | 長いIDを優先（v3.0.0+ ソフト復号で実用的） |
| 32 | 31 | 15シンボル | **31シンボル** | 32文字 | JPEG Q≈70 まで実証済（v3.0.0+ ソフト復号併用前提） |
| 50 | 13 | 6シンボル | **13シンボル** | 50文字 | **非推奨**：耐性が大幅に低下 |
...
`regions` オプションで画像を複数領域に分割し、各領域に同じウォーターマークを埋め込みます。抽出時は領域ごとに独立に抽出したソフト値を合算（Maximal-Ratio Combining）してから RS 復号するため、次の効果が得られます。

- **部分的なクロップへの耐性向上**: 一部領域が完全に失われても、残った領域から復元可能
- **局所的な画像劣化の平均化**: 一部が強く劣化していても他の領域が補う
- **空間的に偏ったノイズに対する堅牢性**: 信号源ダイバーシティの古典的手法
...
- 各領域に **最低 160×160 px** 必要（内部で 20×20 の 8px ブロックを確保するため）
- 要求された `regions` が画像に収まらない場合、**自動的に入る最大値に縮退**します（最終的に `1` まで）
- 縮退が発生すると `console.warn` で通知されます
- embed と extract は同じ画像サイズ・同じ要求値に対して同じ縮退結果になるため整合性が保たれます

| 画像サイズ | regions=2 | regions=4 | regions=9 |
| :---: | :---: | :---: | :---: |
| 256×256 | 不可 (→1) | 不可 (→1) | 不可 (→1) |
| 320×320 | OK (2×1) | OK (2×2) | 不可 (→4) |
| 512×512 | OK | OK (2×2) | 不可 (→4) |
| 1080×1080 | OK | OK | OK (3×3) |

> **注意:** `regions` は埋め込み時と抽出時で**必ず同じ値**を指定してください。埋め込み後に画像が大幅にリサイズされると縮退の結果が変わり抽出が失敗することがあります。
...
- **UI フレームワーク:** React + Vite + Tailwind CSS
- **動画処理:** WebAssembly 版 FFmpeg (`@ffmpeg/ffmpeg`) — ブラウザ内で完結
- **ライブラリ参照:** `watermark-lib/src/browser.ts` 経由 (Node.js 専用 API は除外)
- **状態管理:** React `useState`（`forensicOptions`, `fskOptions`, `llmOptions`, `secureIdLength`, `payloadSymbols`）

UI 側のコードは `forensicOptions` を構築してそのままライブラリ関数 (`embedForensicImage` 等) に渡すだけの「薄いラッパー」設計のため、ライブラリのデフォルト値変更がそのまま UI に反映されます。

### v3.0.0 における UI の挙動

WEB UI は v3.0.0 で**コードを変更せずに自動的に堅牢性アップグレード**の恩恵を受けます:

- **ソフト復号 (`softDecoding: true`)**: ライブラリ側のデフォルト ON のため、解析タブで JPEG 圧縮済みファイルを開いた際の検出率が向上
- **構造化インタリーブ**: 埋め込み・抽出の両方で自動適用。UI から見える振る舞いは変わらないが、内部のビット配置が v2 とは互換性なし
- **payloadSymbols の上限**: UI のセレクトボックスは `[10, 15, 22, 30, 40]` を提供。v3.0.0 ではソフト復号により `30` が「長いID向け」から「実用域」に格上げ。`40` は依然「非推奨」だが v2 よりは現実的になった
- **`erasureThreshold` のオーバーライド**: UI 上で公開していないオプション。`extractForensicImage` を直接呼び出す場合に指定可能（デフォルト `0.35`）。ほとんどのケースでデフォルトで十分

> **互換性注意:** v2.x で生成したウォーターマーク画像を v3.0.0 の WEB UI で開くと検出に失敗します。WEB UI の `package.json` を v2 系に pin するか、CLI で v2 を併用してください。
...
`watermark-lib/` のソース変更は Vite の依存解決により即座に反映されます (`src/App.tsx` から `../watermark-lib/src/browser` を相対パスで参照しているため)。
